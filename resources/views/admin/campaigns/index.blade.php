...
<div class="max-w-7xl mx-auto space-y-6">
    ...
    <div>
        <nav class="-mb-px flex space-x-8">
            @foreach(['queued' => 'Queued', 'sending' => 'Sending', 'completed' => 'Completed', 'failed' => 'Failed'] as $key => $label)
            ...
            <tbody class="divide-y">
                ...
                <tr class="hover:bg-gray-50 transition group">
                    ...
                    <td class="p-4">
                        ...
                        <span class="mt-1 inline-block px-2 py-0.5 rounded text-[10px] font-bold uppercase border
                    ...
                    <td class="p-4">
                        @php 
                            $percent = $campaign->total_emails > 0 ? ($campaign->sent / $campaign->total_emails) * 100 : 0; 
                            $isFinished = $campaign->sent >= $campaign->total_emails && $campaign->total_emails > 0;
                        @endphp

                        <div class="flex justify-between text-xs font-bold text-gray-600 mb-1">
                            <span>{{ number_format($campaign->sent) }} / {{ number_format($campaign->total_emails) }}</span>
                    ...
                    <td class="p-4">
                        ...
                        @if(in_array($campaign->status, ['queued', 'sending', 'paused']))
                            <form action="{{ route('admin.campaigns.assign', $campaign) }}" method="POST" class="flex flex-col gap-1">
                                @csrf
                                <div class="flex gap-2">
                                    <select name="smtp_id" class="text-xs border-gray-300 rounded bg-gray-50 w-full focus:ring-blue-500">
                                        <option value="">-- Assign Server --</option>
                                        @foreach($smtps as $smtp)
                                            <option value="{{ $smtp->id }}" {{ $campaign->smtp_id == $smtp->id ? 'selected' : '' }}>
                                                {{ $smtp->name }} (Lim: {{ $smtp->daily_limit }})
                                            </option>
                                        @endforeach
                                    </select>
                                    <button class="bg-blue-600 text-white px-3 py-1 rounded text-xs font-bold hover:bg-blue-700 whitespace-nowrap shadow-sm">
